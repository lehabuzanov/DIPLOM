from django.urls import path

from sem_corpus.apps.corpus.views import (
    AddArticleToSubcorpusView,
    ArticleDetailView,
    ArticleFileAccessView,
    ArticleHighlightDataView,
    ArticleHighlightDeleteView,
    ArticleHighlightUpdateView,
    ArticleListView,
    ArticleTextDataView,
    EditorArticleUploadView,
    ExportSearchResultsView,
    GeographyDataView,
    GeographyDashboardView,
    IssueDetailView,
    IssueListView,
    SaveQueryView,
    SavedQueryDeleteView,
    SavedQueryListView,
    SavedQueryRunView,
    SavedQueryUpdateView,
    SavedSubcorpusCreateView,
    SavedSubcorpusArticleAddView,
    SavedSubcorpusArticleRemoveView,
    SavedSubcorpusArticleRestoreView,
    SavedSubcorpusDeleteView,
    SavedSubcorpusDetailView,
    SavedSubcorpusFilterUpdateView,
    SavedSubcorpusListView,
    SavedSubcorpusRefreshView,
    SavedSubcorpusUpdateView,
    SearchView,
    SuggestionView,
)

app_name = "corpus"

urlpatterns = [
    path("issues/", IssueListView.as_view(), name="issue-list"),
    path("issues/<int:pk>/", IssueDetailView.as_view(), name="issue-detail"),
    path("geography/", GeographyDashboardView.as_view(), name="geography"),
    path("geography/data/", GeographyDataView.as_view(), name="geography-data"),
    path("articles/", ArticleListView.as_view(), name="article-list"),
    path("articles/upload/", EditorArticleUploadView.as_view(), name="editor-upload"),
    path("articles/<str:slug>/text/", ArticleTextDataView.as_view(), name="article-text-data"),
    path("articles/<str:slug>/highlights/", ArticleHighlightDataView.as_view(), name="article-highlights"),
    path("articles/<str:slug>/", ArticleDetailView.as_view(), name="article-detail"),
    path("articles/<int:pk>/add-to-subcorpus/", AddArticleToSubcorpusView.as_view(), name="article-add-to-subcorpus"),
    path("highlights/<int:pk>/delete/", ArticleHighlightDeleteView.as_view(), name="article-highlight-delete"),
    path("highlights/<int:pk>/edit/", ArticleHighlightUpdateView.as_view(), name="article-highlight-edit"),
    path("files/<int:pk>/", ArticleFileAccessView.as_view(), name="article-file"),
    path("search/", SearchView.as_view(), name="search"),
    path("suggest/<str:kind>/", SuggestionView.as_view(), name="suggest"),
    path("search/export/csv/", ExportSearchResultsView.as_view(), name="search-export-csv"),
    path("search/save/", SaveQueryView.as_view(), name="save-query"),
    path("queries/", SavedQueryListView.as_view(), name="saved-query-list"),
    path("queries/<int:pk>/run/", SavedQueryRunView.as_view(), name="saved-query-run"),
    path("queries/<int:pk>/edit/", SavedQueryUpdateView.as_view(), name="saved-query-edit"),
    path("queries/<int:pk>/delete/", SavedQueryDeleteView.as_view(), name="saved-query-delete"),
    path("subcorpora/", SavedSubcorpusListView.as_view(), name="subcorpus-list"),
    path("subcorpora/new/", SavedSubcorpusCreateView.as_view(), name="subcorpus-create"),
    path("subcorpora/<int:pk>/", SavedSubcorpusDetailView.as_view(), name="subcorpus-detail"),
    path("subcorpora/<int:pk>/filters/", SavedSubcorpusFilterUpdateView.as_view(), name="subcorpus-filter-update"),
    path("subcorpora/<int:pk>/articles/add/", SavedSubcorpusArticleAddView.as_view(), name="subcorpus-article-add"),
    path(
        "subcorpora/<int:pk>/articles/<int:article_pk>/remove/",
        SavedSubcorpusArticleRemoveView.as_view(),
        name="subcorpus-article-remove",
    ),
    path(
        "subcorpora/<int:pk>/articles/restore/",
        SavedSubcorpusArticleRestoreView.as_view(),
        name="subcorpus-article-restore",
    ),
    path("subcorpora/<int:pk>/edit/", SavedSubcorpusUpdateView.as_view(), name="subcorpus-edit"),
    path("subcorpora/<int:pk>/refresh/", SavedSubcorpusRefreshView.as_view(), name="subcorpus-refresh"),
    path("subcorpora/<int:pk>/delete/", SavedSubcorpusDeleteView.as_view(), name="subcorpus-delete"),
]
